...
from operator import icc_rankings, push_to_xcom, run_bq_query, run_bq_sp, fetch_stats, fetch_player_info, load_gcs_to_bigquery
import logging
import re    
from airflow.operators.python import get_current_context
# Get the current DAG run_id
...
def find_missing_player_ids(**kwargs):
    ...
    if player_ids:
        logging.info(f"✅Pulled no.of missing_player_ids: {len(player_ids)}")
        query = f"""
            SELECT player_id 
        ...
        if missing_info_ids:
            ...
            logging.info("✅No missing player_ids found, skipping to end_task")
...
with DAG(
    ...
    start_task = PythonOperator(
        ...
        python_callable=lambda: logging.info("DAG execution started."),
    )
    ...
    sp_update_ranking = PythonOperator(
        ...
        op_kwargs = {
            ...
            "params" : get_current_context()['run_id']
        }
    )
    ...
    end_task = PythonOperator(
        ...
        python_callable=lambda: logging.info("DAG execution completed."),
    )
